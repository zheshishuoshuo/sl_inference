...
def fit_alphasps(mock_lens_data, center_mass=11.4):
    """
    拟合 alpha_sps ~ (logM - 11.4)
    """
    logM = np.asarray(mock_lens_data['logM_star_sps'])
    logRe = np.asarray(mock_lens_data['logRe'])

    x1 = logM - center_mass
    X = x1.reshape(-1, 1)

    reg = LinearRegression().fit(X, logRe)
    resid = logRe - reg.predict(X)
    sigma_alpha = float(resid.std(ddof=2))

    return {
        "mu_alpha": float(reg.intercept_),
