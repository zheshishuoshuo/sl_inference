...
def fit_halo_model_crossfit_fixed_xi(mock_lens_data, center_mass=11.4):
    ...
    """
    logM  = np.asarray(mock_lens_data['logM_star_sps'])
    logMh = np.asarray(mock_lens_data['logM_halo'])
...
def fit_alphasps(mock_lens_data):
    '''
    拟合高斯分布的alphasps
    '''
    logalphasps = mock_lens_data['logalpha_sps']
    mu_alpha = np.mean(logalphasps)
    sigma_alpha = np.std(logalphasps, ddof=1)  # ddof=1 for sample std
